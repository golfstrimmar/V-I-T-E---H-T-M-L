...
let indent = 0;
// const title = document.querySelector("#bunner-title");
const title = [...document.querySelectorAll(".bunner-title ")];

title.forEach((item) => {
  let temp = [...item.querySelectorAll("span")];
  for (let i = 0; i < temp.length; i++) {
    temp[i].style.marginTop = indent + "px";
    indent += -1;
  }
  indent = 0;
});
